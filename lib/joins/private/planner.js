...
module.exports = function planner(options) {
  ...
  if (_.isUndefined(options.getPk) || !_.isFunction(options.getPk)) {
    throw new Error(
      'Options must contain a getPk function that accepts a single argument - modelName.'
    );
  }
  ...
  var determineStrategy = function determineStrategy(instructions) {
    ...
    if (!parentTableName) {
      throw new Error(
        'Unable to find a parentTableName in ' +
          util.inspect(instructions, false, 3)
      );
    }

    if (!childTableName) {
      throw new Error(
        'Unable to find a childTableName in ' +
          util.inspect(instructions, false, 3)
      );
    }
    ...
    try {
      ...
      throw new Error(
        'Error finding a primary key attribute for ' +
          parentTableName +
          '\n\n' +
          e.stack
      );
    ...
    if (strategy === strategies.VIA_JUNCTOR) {
      ...
      try {
        ...
        throw new Error(
          'Error finding a primary key attribute for junction table: ' +
            _.first(instructions).child +
            '\n\n' +
            e.stack
        );
      }

...
module.exports = function detectChildrenRecords(primaryKeyAttr, records) {
  ...
  _.each(records, function searchForChildren(record) {
    ...
    if (!recordPk) {
      throw new Error(
        'Could not find a primary key for a record. Perhaps it wasn\'t selected in the query. The primary key was set to: ' +
          primaryKeyAttr +
          ' and the record returned was: ' +
          util.inspect(record, false, null)
      );
    }

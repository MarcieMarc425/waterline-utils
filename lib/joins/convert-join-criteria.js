...
module.exports = function convertCriteria(options) {
  ...
  if (_.isUndefined(options) || !_.isPlainObject(options)) {
    throw new Error(
      'Invalid options argument. Options must contain: tableName, schemaName, getPk, and criteria.'
    );
  }

  if (!_.has(options, 'query') || !_.isPlainObject(options.query)) {
    throw new Error(
      'Invalid option used in options argument. Missing or invalid query.'
    );
  }

  if (!_.has(options, 'getPk') || !_.isFunction(options.getPk)) {
    throw new Error(
      'Invalid option used in options argument. Missing or invalid getPk function.'
    );
  }
  ...
  if (!_.has(query, 'joins')) {
    try {
      ...
      throw new Error(
        'There was an error converting the Waterline Query into a Waterline Statement: ' +
          e.message
      );
    }
  ...
  _.each(instructions, function processJoins(val, key) {
    ...
    _.each(val.instructions, function checkForCriteria(joinSet) {
      ...
      if (
        _.has(joinSet.criteria, 'sort') ||
        _.has(joinSet.criteria, 'limit') ||
        _.has(joinSet.criteria, 'skip') ||
        _.has(joinSet.criteria, 'where')
      ) {
        slowJoin = true;
        slowJoinAliases.push(key);
      }
  ...
  if (!slowJoin) {
    ...
    try {
      parentStatement.where = Helpers.expandCriteria(
        parentStatement.where,
        query.using
      );
    } catch (e) {
      throw new Error(
        'There was an error trying to expand the criteria used in the WHERE clause. Perhaps it is invalid? ' +
          e.stack
      );
    }
  ...
  try {
    ...
    fastJoinInstructionArray = _.map(
      _.keys(fastInstructions),
      function parseAlias(alias) {
        return instructions[alias];
      }
    );
  ...
  _.each(slowJoinAliases, function buildJoinTemplate(alias) {
    ...
    // query gets built.
    if (
      _.has(parentInstructions.criteria, 'skip') &&
      parentInstructions.criteria.skip === 0
    ) {
      delete parentInstructions.criteria.skip;
    }

    if (
      _.has(parentInstructions.criteria, 'limit') &&
      parentInstructions.criteria.limit === maxInt
    ) {
    ...
    var paginated =
      _.get(parentInstructions.criteria, 'skip', 0) !== 0 ||
      _.get(parentInstructions.criteria, 'limit', Number.MAX_SAFE_INTEGER) !==
      ...
      (function generateTemplate() {
        ...
        try {
          (function buildUpConvertCriteria() {
            var convertObj = {
              model:
                parentInstructions.child +
                ' as ' +
                parentInstructions.childAlias,
              method: 'find',
        ...
        try {
          ...
          throw new Error(
            'There was an issue getting the primary key attribute from ' +
              parentInstructions.child +
              ' are ' +
        ...
        whereTemplate[
          parentInstructions.childAlias + '.' + parentInstructions.childKey
        ] = {
          in: [],
        };
    ...
    var childPaginated =
      _.has(childInstructions.criteria, 'skip') ||
      _.has(childInstructions.criteria, 'limit');
    ...
    if (strategy === 3 && !childPaginated) {
      (function generateTemplate() {
        ...
        statement.select = _.map(
          childInstructions.criteria.select,
          function normalizeSelect(column) {

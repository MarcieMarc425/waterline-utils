...
module.exports = function runAlterStrategy(orm, cb) {
  ...
  if (
    ...
    !process.env.ALLOW_UNSAFE_MIGRATIONS
  ) {
    return cb(
      new Error(
        '`migrate: \'alter\'` strategy is not supported in production, please change to `migrate: \'safe\'`.'
      )
    );
  }
  ...
  async.each(
    _.keys(orm.collections),
    function simultaneouslyMigrateEachModel(modelIdentity, next) {
      ...
      try {
        _.each(
          WLModel.schema,
          function parseAttribute(wlsAttrDef, wlsAttrName) {
                ...
                function defineCallback(err) {
                  ...
                  WLModel.createEach(backupRecords)
                    ...
                    .exec(function createEachCallback(err) {
                      ...
                      if (
                        WLModel.schema[primaryKeyAttrName].autoMigrations
                          .autoIncrement !== true
                      ) {
                        return next();
                      }
                      ...
                      if (!_.has(WLAdapter, 'setSequence')) {
                        ...
                        console.warn(
                          '\n' +
                            ...
                            'of this model (`' +
                            WLModel.identity +
                            '`), this adapter does not support the\n' +
                      ...
                      var primaryKeyColumnName =
                        WLModel.schema[primaryKeyAttrName].columnName;
                      var sequenceName =
                        WLModel.tableName + '_' + primaryKeyColumnName + '_seq';
                      var sequenceValue = lastRecord[primaryKeyColumnName];

                      WLAdapter.setSequence(
                        datastoreName,
                        sequenceName,
                        sequenceValue,
                        function setSequenceCb(err) {
                          if (err) {
                            return next(err);
                          }

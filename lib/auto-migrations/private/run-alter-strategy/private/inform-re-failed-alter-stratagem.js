 ...
 */
module.exports = function informReFailedAlterStratagem(
  err,
  operationName,
  modelIdentity,
  backupRecords,
  ...
  var timeSeriesUniqueishSuffixPiece = Math.floor(
    (Date.now() % 10000000) / 1000
  );
  var relPathToLogFile =
    '.tmp/automigration.' +
    modelIdentity +
    '.' +
    timeSeriesUniqueishSuffixPiece +
    '.log';
  ...
  if (isCoercionFailure) {
    message +=
      ...
      'Some existing `' +
      modelIdentity +
      '` record(s) couldn\'t be adjusted automatically to match ' +
      ...
      'Details:\n' +
      '```\n' +
      'Failed to reinsert migrated data. ' +
    ...
    message +=
      ...
      'Some existing `' +
      modelIdentity +
      '` record(s) violated a uniqueness constraint when attempting to ' +
  ...
  // Otherwise this was some kind of weird, unexpected error.  So we use the catch-all approach:
    ...
    if (_.isError(err) && _.keys(err).length === 0) {
      formattedDisplayError = err.stack;
    } else if (_.isError(err)) {
      formattedDisplayError = util.inspect(err);
    } else if (_.isString(err)) {
      formattedDisplayError = err;
    } else {
      formattedDisplayError = util.inspect(err, { depth: 5 });
    }

    message +=
      'an unexpected error when performing the `' +
      operationName +
      '` step.  ' +
  ...
  message +=
    ...
    'Any existing `' +
    modelIdentity +
    '` records were deleted, but your data from OTHER models ' +
  ...
  logFileContents +=
    'were originally stored in the `' + modelIdentity + '` model.\n';

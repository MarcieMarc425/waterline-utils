...
module.exports = function normalizeDatastoreConfig(
  ...
  assert(
    ...
    'If provided, 2nd argument should be a string (e.g. "mongodb") representing the prefix for the expected protocol.'
  );
  ...
  _.each(Object.keys(dsConfig), function (key) {
    ...
    if (
      whitelist &&
      !_.includes(whitelist, key) &&
      !_.includes(BASELINE_PROPS, key)
    ) {
    ...
    if (hasDatabaseOverride) {
      ...
      throw flaverr(
        ...
        new Error(
          'No `url` was specified, and no appropriate connection URL can be inferred (tried to use ' +
            '`database: ' +
            util.inspect(dsConfig.database) +
            '`).\n' +
            '--\n' +
  ...
  // Otherwise, normalize & parse the connection URL.
    ...
    if (!parsedConnectionStr.protocol) {
      throw flaverr(
        'E_BAD_CONFIG',
        new Error(
          'Could not parse provided URL (' +
            util.inspect(dsConfig.url, { depth: 5 }) +
            ').\n' +
        ...
        throw flaverr(
          ...
          new Error(
            'Provided URL (' +
              ...
              'If included, the protocol must be "' +
              expectedProtocolPrefix +
              '://".\n' +
              '--\n' +
              '(See http://sailsjs.com/config/datastores#?the-connection-url for more info.)'
    ...
    if (
      parsedConnectionStr !== 'db:' &&
      dsConfig.protocolPrefix === undefined
    ) {
    ...
    try {
      ...
      throw flaverr(
        'E_BAD_CONFIG',
        new Error(
          'Could not parse query string from URL: `' +
            dsConfig.url +
            '`.  ' +
            'Details: ' +
            e.stack
    ...
    try {
      ...
      _.each(miscOptsInUrlQs, function (val, key) {
        if (whitelist && !_.includes(whitelist, key)) {
          throw flaverr(
            'E_BAD_CONFIG',
            new Error(
              'Unrecognized option (`' +
                key +
                '`) specified in query string of connection URL.\n' +
        ...
        if (_.includes(BASELINE_PROPS, key)) {
      ...
      switch (e.code) {
        case 'E_BAD_CONFIG':
          throw flaverr(
            'E_BAD_CONFIG',
            new Error(
              'Could not process connection url.  ' +
                e.message +
                '\n' +
                '--\n' +

...
var tokenizeObject = function tokenizeObject(
  obj,
  processor,
  parent,
  isSubQuery,
  ...
  _.each(_.keys(obj), function tokenizeKey(key, idx) {
    ...
    if (isIdentitifier) {
      ...
      if (inWhere && _.includes(WHERE_EXEMPT, key)) {
        results.push({
          type: 'KEY',
          value: key,
        });
  ...
  _.each(value, function processJoinInstructions(joinInstructions) {
    ...
    if (_.isArray(joinInstructions.on)) {
      ...
      (function buildJoinResults() {
        ...
        var PARENT_COLUMN =
          joinInstructions.on[_.first(_.keys(joinInstructions.on))];
...
//  ╠═╣║ ╦║ ╦╠╦╝║╣ ║ ╦╠═╣ ║ ║║ ║║║║╚═╗
//  ╩ ╩╚═╝╚═╝╩╚═╚═╝╚═╝╩ ╩ ╩ ╩╚═╝╝╚╝╚═╝
var processAggregations = function processAggregations(
  value,
  aggregation,
  results
) {

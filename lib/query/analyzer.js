...
module.exports = function analyzer(tokens) {
  ...
  var OUTPUTTING_CONDITIONS = ['NOT', 'IN', 'NOTIN', 'AND'];
  ...
  var result = (function analyzer(tokens) {
    ...
    _.each(tokens, function (token) {
      switch (token.type) {
        ...
        case 'IDENTIFIER':
          ...
          if (_.includes(WRAPPED_IDENTIFIERS, token.value)) {
            pushStack();
          }
          break;
        ...
        case 'CONDITION':
          if (_.includes(OUTPUTTING_CONDITIONS, token.value)) {
            curChunk.push(token);
          }
          break;
        ...
        case 'SUBQUERY':
          if (
            unionSubqueryStack.length === 0 ||
            _.last(unionSubqueryStack) === 'SUBQUERY'
          ) {
            curChunk.push(token);
            pushStack();
          }
    ...
    if (stack.length > 1) {
      throw new Error(
        'Consistency violation: final stack in analyzer contains more than one item.  Stack is: ',
        require('util').inspect(stack, { depth: null })
      );
    }

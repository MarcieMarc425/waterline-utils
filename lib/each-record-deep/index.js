...
module.exports = function eachRecordDeep(
  records,
  iteratee,
  arePhysical,
  modelIdentity,
  ...
  if (!_.isFunction(iteratee)) {
    throw new Error(
      'Consistency violation: Expected `iteratee` to be a function.  But instead, got: ' +
        util.inspect(iteratee, { depth: 5 }) +
        ''
    );
  }
  ...
  _.each(records, function iterateRecords(record) {
    if (!_.isObject(record) || _.isArray(record) || _.isFunction(record)) {
      throw new Error(
        'Consistency violation: Expected each item in the `records` array to be a record (a dictionary). But at least one of them is messed up. Record: ' +
    ...
    _.each(
      ParentWLModel.attributes,
      function iterateAttributes(attrDef, attrName) {
        ...
        if (attrDef.model) {
          ...
          if (
            !_.isObject(valueInParentRecord) ||
            _.isArray(valueInParentRecord) ||
            _.isFunction(valueInParentRecord)
          ) {
            return;
          }

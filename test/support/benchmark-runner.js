...
module.exports = function runBenchmarks(name, testFns) {
  ...
  suite
    ...
    .on('complete', function () {
      ...
      var mean = {
        fastest: Benchmark.formatNumber(
          fastestMean < 1 ? fastestMean.toFixed(2) : Math.round(fastestMean)
        ),
        slowest: Benchmark.formatNumber(
          slowestMean < 1 ? slowestMean.toFixed(2) : Math.round(slowestMean)
        ),
      };
